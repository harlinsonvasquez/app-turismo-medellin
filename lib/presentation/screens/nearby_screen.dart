      ...
      body: Column(
        children: [
          Container(
            ...
            child: Stack(
              children: [
                Positioned.fill(child: ClipRRect(borderRadius: BorderRadius.circular(AppConstants.radiusXL), child: CustomPaint(painter: _FakeMapPainter()))),
                Center(
                  child: Column(
                    ...
                    children: [
                      Container(
                        ...
                        decoration: BoxDecoration(color: AppColors.primary, shape: BoxShape.circle, boxShadow: [BoxShadow(color: AppColors.primary.withOpacity(0.4), blurRadius: 16, spreadRadius: 4)]),
                        child: const Icon(Icons.my_location_rounded, color: Colors.white, size: 28),
                      ),
                      const SizedBox(height: 8),
                      Container(
                        padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                        decoration: BoxDecoration(color: Colors.white.withOpacity(0.9), borderRadius: BorderRadius.circular(AppConstants.radiusFull)),
                        child: const Text('?? Medellin centro', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 13, color: AppColors.textPrimary)),
                      ),
                    ],
                  ),
                ),
          ...
          Padding(
            ...
            child: Row(
              children: [
                Text('${places.length} lugares cercanos', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
                const Spacer(),
                Row(
                    ...
                    return Padding(
                      ...
                      child: GestureDetector(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(color: selected ? AppColors.primary : AppColors.surfaceVariant, borderRadius: BorderRadius.circular(AppConstants.radiusFull)),
                          child: Text(option, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: selected ? Colors.white : AppColors.textPrimary)),
                        ),
                      ),
                    );
                  }).toList(),
...
class _FakeMapPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final paint = Paint()..color = const Color(0xFFBCE0A0)..strokeWidth = 1.5..style = PaintingStyle.stroke;
    for (int i = 1; i < 5; i++) {
      canvas.drawLine(Offset(0, size.height * i / 5), Offset(size.width, size.height * i / 5), paint);
    }
    for (int i = 1; i < 6; i++) {
      canvas.drawLine(Offset(size.width * i / 6, 0), Offset(size.width * i / 6, size.height), paint);
    }
  }
