            ...
            flexibleSpace: FlexibleSpaceBar(
              background: Container(
                decoration: const BoxDecoration(
                  gradient: LinearGradient(colors: [AppColors.primaryDark, AppColors.primaryLight], begin: Alignment.topLeft, end: Alignment.bottomRight),
                ),
                child: SafeArea(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(user?.name ?? 'Invitado', style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w700)),
                      const SizedBox(height: 4),
                      Text(user?.email ?? 'Inicia sesion para sincronizar tu viaje', style: TextStyle(color: Colors.white.withOpacity(0.8), fontSize: 13)),
                    ],
                  ),
                ),
              ),
                  ...
                  if (!auth.isAuthenticated) ...[
                    Container(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const Text('Activa tu cuenta', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800)),
                          const SizedBox(height: 8),
                          const Text('Necesitas iniciar sesion para guardar favoritos y persistir tus itinerarios.'),
                          const SizedBox(height: 16),
                          Row(
                            children: [
                              Expanded(child: ElevatedButton(onPressed: () => context.go(AppConstants.routeLogin), child: const Text('Iniciar sesion'))),
                              const SizedBox(width: 12),
                              Expanded(child: OutlinedButton(onPressed: () => context.go(AppConstants.routeRegister), child: const Text('Crear cuenta'))),
                            ],
                          ),
                        ],
                      ),
...
class _SectionTitle extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Text(text, style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w700));
  }
}
  ...
  Widget build(BuildContext context) {
    return Expanded(
      child: Container(
        ...
        child: Column(
          children: [
            Text(value, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: AppColors.primary), textAlign: TextAlign.center),
            const SizedBox(height: 4),
            Text(label, style: const TextStyle(fontSize: 10, color: AppColors.textTertiary, height: 1.3), textAlign: TextAlign.center),
          ],
        ),
      ),
    );
      ...
      child: Column(
        children: children.asMap().entries.map((entry) {
          return Column(
            children: [
              entry.value,
              if (entry.key < children.length - 1) const Divider(height: 1, indent: 56),
            ],
          );
        }).toList(),
      ),
