...
/// Representa el perfil autenticado devuelto por `/api/auth/login`,
/// `/api/auth/register` y `/api/auth/me`.
class UserModel {
  final String id;
  final String name;
  final String email;
...
/// Modela un consejo de seguridad devuelto por `/api/safety-tips`.
class SafetyTipModel {
  ...
  static SafetyTipCategory _categoryFromText(String title, String description) {
    ...
    if (text.contains('emergency') || text.contains('123'))
      return SafetyTipCategory.emergency;
    if (text.contains('taxi') || text.contains('transport'))
      return SafetyTipCategory.transport;
    if (text.contains('zone') ||
        text.contains('poblado') ||
        text.contains('centro')) return SafetyTipCategory.zones;
    if (text.contains('scam') || text.contains('estafa'))
      return SafetyTipCategory.scam;
    return SafetyTipCategory.general;
  }

  static String _iconForCategory(SafetyTipCategory category) =>
      switch (category) {
...
/// Modela los planes comerciales obtenidos desde `/api/membership-plans`.
class MembershipPlanModel {
  ...
  factory MembershipPlanModel.fromJson(Map<String, dynamic> json) {
    ...
    try {
      features = (jsonDecode(featuresJson) as List<dynamic>)
    ...
    return MembershipPlanModel(
      ...
      name: name,
      price:
          '\$${((json['monthlyPrice'] as num?)?.toDouble() ?? 0).round()} COP',
      billingPeriod: 'por mes',
      features: features,
      isPopular: name.toLowerCase() == 'pro',
      badgeColor: name.toLowerCase() == 'premium'
          ? 'F39C12'
...
/// Representa un favorito persistido en `/api/favorites`.
class FavoriteModel {
  final String id;
  ...
  factory FavoriteModel.fromJson(Map<String, dynamic> json) => FavoriteModel(
        ...
        savedAt: DateTime.tryParse(json['createdAt']?.toString() ?? '') ??
            DateTime.now(),
      );
}
