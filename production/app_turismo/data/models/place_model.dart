...
/// Representa cada imagen asociada a un lugar turistico que llega desde
/// `/api/places` y mantiene el orden definido por el backend.
...
/// Modela un lugar visible para el turista y adapta la respuesta de
/// `/api/places` y `/api/places/{id}` para la interfaz Flutter.
class PlaceModel {
  final String id;
  ...
  factory PlaceModel.fromJson(Map<String, dynamic> json) {
    ...
    return PlaceModel(
      ...
      openingHours:
          json['openingHoursJson']?.toString() ?? 'Horario por confirmar',
      ...
      safetyNote: safe
          ? 'Zona considerada segura para turistas.'
          : 'Manten precaucion con tus pertenencias.',
      isFeatured: json['featured'] == true,
      isPopular: json['featured'] == true ||
          ((json['rating'] as num?)?.toDouble() ?? 0) >= 4.5,
      isActive: json['active'] != false,
    );
  }

  /// Devuelve la mejor imagen disponible para tarjetas y detalle.
  /// Prioriza la portada y luego la primera URL no vacia.
  String? get imagenPrincipalUrl {
    for (final image in images) {
      if (image.cover && image.imageUrl.trim().isNotEmpty) {
