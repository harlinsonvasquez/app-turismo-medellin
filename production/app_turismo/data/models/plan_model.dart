...
/// Modela una actividad puntual dentro del itinerario generado o guardado.
class ItineraryBlock {
  ...
  factory ItineraryBlock.fromJson(Map<String, dynamic> json) => ItineraryBlock(
        ...
        estimatedCost:
            _formatMoney((json['estimatedCost'] as num?)?.toDouble()),
...
/// Agrupa las actividades del itinerario por dia y franja horaria para la UI.
class DayPlan {
  final int dayNumber;
  ...
  String get dailyBudget {
    final total =
        [...morning, ...afternoon, ...night].fold<num>(0, (sum, item) {
  ...
  String get safetyNote {
    final safety =
        night.where((item) => item.itemType == ItineraryItemType.safetyNote);
    if (safety.isNotEmpty) return safety.first.title;
    return 'Mantente en zonas concurridas y usa transporte verificado.';
  }
}

/// Representa el itinerario completo que viaja entre Flutter y `/api/itineraries`.
class TripPlanModel {
  final String id;
  final String title;
  final int days;
  ...
  factory TripPlanModel.fromJson(Map<String, dynamic> json) {
    ...
    final dayPlans = groupedDays.entries.map((entry) {
      ...
      return DayPlan(
        dayNumber: entry.key,
        morning:
            blocks.where((e) => e.period == ItineraryPeriod.morning).toList(),
        afternoon:
            blocks.where((e) => e.period == ItineraryPeriod.afternoon).toList(),
    ...
    return TripPlanModel(
      ...
      dayPlans: dayPlans,
      estimatedTotal:
          '\$${((json['totalBudget'] as num?)?.toDouble() ?? 0).round()} COP',
      transportSummary: 'Metro, apps de transporte y recorridos a pie',
      generalNotes: [
        if ((json['notes']?.toString() ?? '').isNotEmpty)
          json['notes'].toString(),
...
/// Reune la entrada que el usuario define antes de llamar a
...
/// Estructura auxiliar para pantallas que mezclan guardados de lugares y planes.
