...
void main() {
  ...
  final almacenamientoToken = AlmacenamientoToken();
  final clienteApi = ApiClient(almacenamientoToken);

  runApp(AppTurismo(apiClient: clienteApi, tokenStorage: almacenamientoToken));
}

/// Punto de composicion principal del frontend.
/// Registra servicios y providers que consumen el backend Spring Boot.
class AppTurismo extends StatelessWidget {
  const AppTurismo(
      {super.key, required this.apiClient, required this.tokenStorage});

  final ApiClient apiClient;
  final AlmacenamientoToken tokenStorage;
  ...
  Widget build(BuildContext context) {
    return MultiProvider(
      providers: [
        ...
        ChangeNotifierProvider(
            create: (_) => PlaceProvider(PlaceService(apiClient))),
        ChangeNotifierProvider(
            create: (_) => EventProvider(EventService(apiClient))),
        ChangeNotifierProvider(
            create: (_) => SafetyProvider(SafetyService(apiClient))),
        ChangeNotifierProvider(
            create: (_) => MembershipProvider(MembershipService(apiClient))),
        ChangeNotifierProvider(
            create: (_) => FavoritesProvider(FavoriteService(apiClient))),
        ChangeNotifierProvider(
            create: (_) => ItineraryProvider(ItineraryService(apiClient))),
      ],
